...
class TransactionListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        # Fetch records where user is either the borrower OR the lender
...
class InitiateReturnView(APIView):
    ...
    def patch(self, request, pk):
        # Ensure only the actual borrower can trigger the return flow
        transaction = get_object_or_404(
            Transaction, id=pk, borrower=request.user)
...
class ConfirmReturnView(APIView):
    ...
    def patch(self, request, pk):
        # Ensure only the actual listing owner can close out the transaction
        transaction = get_object_or_404(Transaction, id=pk, owner=request.user)
