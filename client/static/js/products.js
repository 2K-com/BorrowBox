...
function moveIndicator(element) {
    // Get the position of the clicked element relative to the wrapper
    const rect = element.getBoundingClientRect();
    const parentRect = element.closest('.pagination-wrapper').getBoundingClientRect();
    
    // Set the left position of the purple circle
...
nextBtn.addEventListener('click', () => {
    if (currentPageIndex < items.length - 1) {
        currentPageIndex++;
        moveIndicator(items[currentPageIndex]);
    }
});
